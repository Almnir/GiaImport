...
namespace FtcReportControls.WindowsSystem
{
    ...
    public partial class ProgressDialog : Component
    {
        ...
        /// <summary>
        /// Register handlers without consequent addition
        /// </summary>
        /// <param name="workHandler"></param>
        /// <param name="runWorkerComplete"></param>
        public void RegisterHandlers(DoWorkEventHandler workHandler, RunWorkerCompletedEventHandler runWorkerComplete)
        {
            if (this.DoWork == null)
            {
                this.DoWork += workHandler;
            }
            if (this.RunWorkerCompleted == null)
            {
                this.RunWorkerCompleted += runWorkerComplete;
            }
        }
        ...
        public ProgressDialog(IContainer container)
        {
            ...
            if( !NativeMethods.IsWindowsVistaOrLater )
                Animation = AnimationResource.GetShellAnimation(FtcReportControls.WindowsSystem.ShellAnimation.FlyingPapers);
        }
        ...
        /// </summary>
        /// <value>
        /// One of the values of <see cref="FtcReportControls.WindowsSystem.ProgressBarStyle"/>. 
        /// The default value is <see cref="FtcReportControls.WindowsSystem.ProgressBarStyle.ProgressBar"/>.
        /// </value>
        /// <remarks>
        /// <note>
        ...
        ///   progress bar will be hidden completely if this property is <see cref="FtcReportControls.WindowsSystem.ProgressBarStyle.MarqueeProgressBar"/>.
        /// </note>
        /// <para>
        ///   When this property is set to <see cref="FtcReportControls.WindowsSystem.ProgressBarStyle.ProgressBar" />, use the <see cref="ReportProgress(int)"/> method to set
        ///   the value of the progress bar. When this property is set to <see cref="FtcReportControls.WindowsSystem.ProgressBarStyle.MarqueeProgressBar"/>
        ...
        /// </para>
        /// <para>
        ///   This method has no effect is <see cref="ProgressBarStyle"/> is <see cref="FtcReportControls.WindowsSystem.ProgressBarStyle.MarqueeProgressBar"/>
        ///   or <see cref="FtcReportControls.WindowsSystem.ProgressBarStyle.None"/>.
        /// </para>
        /// </remarks>
        ...
        public void ReportProgress(int percentProgress, string text, string description, object userState)
        {
            if( percentProgress < 0 || percentProgress > 100 )
                throw new ArgumentOutOfRangeException("Выполнение выходит за границы прогресса выполнения.");
            if( _dialog == null )
                throw new InvalidOperationException(ErrorStrings.ProgressDialogNotRunningError);
        ...
        private void RunProgressDialog(IntPtr owner, object argument)
        {
            if( _backgroundWorker.IsBusy )
                throw new InvalidOperationException(ErrorStrings.ProgressDialogRunning);
            ...
            if( Animation != null )
            {
                ...
                {
                    throw new InvalidOperationException(string.Format(System.Globalization.CultureInfo.CurrentCulture, ErrorStrings.AnimationLoadErrorFormat, ex.Message), ex);
                }
            ...
            Interop.ProgressDialogFlags flags = FtcReportControls.WindowsSystem.Interop.ProgressDialogFlags.Normal;
            if( owner != IntPtr.Zero )
                flags |= FtcReportControls.WindowsSystem.Interop.ProgressDialogFlags.Modal;
            switch( ProgressBarStyle )
            {
            case ProgressBarStyle.None:
                flags |= FtcReportControls.WindowsSystem.Interop.ProgressDialogFlags.NoProgressBar;
                break;
            case ProgressBarStyle.MarqueeProgressBar:
                if( NativeMethods.IsWindowsVistaOrLater )
                    flags |= FtcReportControls.WindowsSystem.Interop.ProgressDialogFlags.MarqueeProgress;
                else
                    flags |= FtcReportControls.WindowsSystem.Interop.ProgressDialogFlags.NoProgressBar; // Older than Vista doesn't support marquee.
                break;
            }
            if( ShowTimeRemaining )
                flags |= FtcReportControls.WindowsSystem.Interop.ProgressDialogFlags.AutoTime;
            if( !ShowCancelButton )
                flags |= FtcReportControls.WindowsSystem.Interop.ProgressDialogFlags.NoCancel;
            if( !MinimizeBox )
                flags |= FtcReportControls.WindowsSystem.Interop.ProgressDialogFlags.NoMinimize;
        ...
        private void _backgroundWorker_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            ...
            {
                Console.WriteLine("Ошибка при попытке изменения состояния индикатора процесса:\r\n{0}", ex.Message);
            }
            return;
        }
    }
