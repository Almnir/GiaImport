...
namespace FtcReportControls.WindowsSystem
{
    ...
    public class VistaOpenFileDialog : VistaFileDialog
    {
        ...
        internal override FtcReportControls.WindowsSystem.Interop.IFileDialog CreateFileDialog()
        {
            return new Interop.NativeFileOpenDialog();
        }

        internal override void SetDialogProperties(FtcReportControls.WindowsSystem.Interop.IFileDialog dialog)
        {
        ...
        internal override void GetResult(FtcReportControls.WindowsSystem.Interop.IFileDialog dialog)
        {
            if( Multiselect )
            {
                FtcReportControls.WindowsSystem.Interop.IShellItemArray results;
                ((FtcReportControls.WindowsSystem.Interop.IFileOpenDialog)dialog).GetResults(out results);
                uint count;
                results.GetCount(out count);
                string[] fileNames = new string[count];
                for( uint x = 0; x < count; ++x )
                {
                    FtcReportControls.WindowsSystem.Interop.IShellItem item;
