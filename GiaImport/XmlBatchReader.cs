    ...
    public class XmlBatchReader
    {
        ...
        public void ProcessFileWithAction(string filePath, Action<XDocument> action)
        {
            ...
            using (var reader = XmlReader.Create(filePath, settings))
            {
                ...
                foreach (var batch in ReadBatch(reader, typeof(object).Name, _batchSize))
                {
                    root.Add(batch);
                    action(batchOfXml);
                    root.RemoveAll();
        ...
        /// <returns></returns>
        public static IEnumerable<List<XElement>> ReadBatch(XmlReader reader, string tagName, int batchSize)
        {
